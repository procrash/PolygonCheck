...
public:
    ...
    int depth();
    Btree insert(K, V);
    Btree insertAll(std::vector<std::pair<K, V>>);
    Node* getRoot();
    Node* beginning();
    Node* end();
    Node* get(K);
...
template <typename K, typename V>
        Btree<K, V> Btree<K, V>::insertAll(std::vector<std::pair<K, V>> v) {
    for (auto &p : v) {
...
template <typename K, typename V>
bool Btree<K, V>::containsAll(std::vector<K> keys) {
    bool all = true;
    for (auto &key : keys) {
        if (!contains(key)) {
            all = false;
